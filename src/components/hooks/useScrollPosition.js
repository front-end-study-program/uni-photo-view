...
// 触边运动反馈
const rebound = (start, bound, callback) =>
  easeOutMove(
    start,
    bound,
    callback,
...
export default function useScrollPosition (
  ...
  return (
    ...
    if (moveTime >= maxTouchTime || safeScale !== scale || Math.abs(lastScale - scale) > 1) {
      ...
      if (targetY !== null) {
        easeOutMove(y, targetY, callbackY)
      }
      if (safeScale !== scale) {
        easeOutMove(scale, safeScale, callbackS)
      }
      return
    }
    ...
    scrollMove(speedT, (spatial) => {
      ...
      if (isEdgeX && !edgeX) {
        edgeX = true
        if (beginEdgeX) {
          easeOutMove(nextX, currentX, callbackX)
        } else {
          rebound(currentX, nextX + (nextX - currentX), callbackX)
        }
      }
      ...
      if (isEdgeY && !edgeY) {
        edgeY = true
        if (beginEdgeY) {
          easeOutMove(nextY, currentY, callbackY)
        } else {
          rebound(currentY, nextY + (nextY - currentY), callbackY)
        }
      }
      // 同时接触边缘的情况下停止滚动
      if (edgeX && edgeY) {
      ...
      const resultX = edgeX || callbackX(currentX)
      const resultY = edgeY || callbackY(currentY)
